...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // mailto: is resolved by the OS's registered mail handler, not by the
  // browser - Safari and Chrome behave identically here, both just defer to
  // whatever the user has configured (Mail.app, Outlook, the Gmail app on
  // Android/iOS, etc). There's no reliable way to detect from JS whether that
  // will work, so we let the link fire natively everywhere rather than
  // guessing and hijacking it. We only add a clipboard-copy as a silent
  // fallback, with a visible confirmation, in case nothing opens.
